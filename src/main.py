...
def fazer_chute(resposta_label):
    ...
    resposta_label.config(text="O número em que você está pensando é 'maior', 'menor' ou 'igual' a: {}".format((limite_inferior + limite_superior) // 2))
...
def jogo_adivinhacao(numero_min, numero_max):
    ...
    resposta_label = tk.Label(input_frame, text="O número em que você está pensando é 'maior', 'menor' ou 'igual' a: {}".format((limite_inferior + limite_superior) // 2))
